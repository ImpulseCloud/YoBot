...
MeleeManager::MeleeManager(YoAgent & bot)
	: MicroManager(bot)
{
...
void MeleeManager::assignTargets(const std::vector<const sc2::Unit *> & targets)
{
	...
	for (auto & meleeUnit : meleeUnits)
	{
		...
		if (order.getType() == SquadOrderTypes::Attack || order.getType() == SquadOrderTypes::Defend)
		{
			...
			{
				sc2::Point2D fleeTo(m_bot.Observation()->GetStartLocation());
			...
			else
			{
				...
				if (Util::Dist(meleeUnit->pos, order.getPosition()) > 4)
				{
					...
					m_bot.Actions()->UnitCommand(meleeUnit, sc2::ABILITY_ID::MOVE, order.getPosition());
				}
			}
		}
