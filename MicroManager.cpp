...
MicroManager::MicroManager(YoAgent & bot)
	: m_bot(bot)
{
}
...
void MicroManager::execute(const SquadOrder & inputOrder)
{
	...
	std::vector<const sc2::Unit *> allEnemies;

	allEnemies = m_bot.Observation()->GetUnits(sc2::Unit::Alliance::Enemy);

	// if the order is to defend, we only care about units in the radius of the defense
	if (order.getType() == SquadOrderTypes::Defend)
	{
		//auto enemiesNearDefense = m_bot.Observation(sc2::Unit::Alliance::Enemy, [inputOrder](const auto & u){return Util::Dist(u->pos, order.getPosition()) < order.getRadius();} );

		for (auto & enemyUnit : allEnemies)
		{
	...
	{
		for (auto & enemyUnit : allEnemies) //m_bot.UnitInfo().getUnits(Players::Enemy))
		{
			if (Util::Dist(enemyUnit->pos, order.getPosition()) < order.getRadius())
			{
				nearbyEnemies.insert(enemyUnit);
	...
	if (order.getType() == SquadOrderTypes::Attack || order.getType() == SquadOrderTypes::Defend)
	{
		...
		else
		{
			...
			for (auto enemyUnit : targetUnitTags)
			{
				...
				{
					for (const BaseLocation * enemyBaseLocation : m_bot.m_baseLocationManager.getOccupiedBaseLocations(Players::Enemy))
					{
...
{
	sc2::Point2D ourBasePosition = m_bot.Observation()->GetStartLocation();
	int regroupDistanceFromBase = m_bot.m_map.getGroundDistance(regroupPosition, ourBasePosition);
	...
	for (auto unit : m_units)
	{
		...
		int unitDistanceFromBase = m_bot.m_map.getGroundDistance(unit->pos, ourBasePosition);
