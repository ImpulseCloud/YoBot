...
SquadData::SquadData(YoAgent & bot)
	: m_bot(bot)
{
...
void SquadData::clearSquadData()
{
	...
	/*for (auto & kv : m_squads)
	{
		...
		for (auto & unit : squad.getUnits())
		{
			...
			//if (Util::IsWorker(unit)) m_bot.Workers().finishedWithWorker(unit); //not using this with Workers yet
		}
	}*/
...
void SquadData::removeSquad(const std::string & squadName)
{
	...
	{
		//BOT_ASSERT(unit, "null unit");
		//if (Util::IsWorker(unit)) m_bot.Workers().finishedWithWorker(unit);
	}*/
...
{
	//if (!m_bot.Config().DrawSquadInfo) return; //not using Config file yet
	...
	for (auto & kv : m_squads)
	{
		...
		m_bot.m_map.drawSphere(order.getPosition(), 5, sc2::Colors::Red);
		m_bot.m_map.drawText(order.getPosition(), squad.getName(), sc2::Colors::Red);

		for (auto unit : units)
		{
			//BOT_ASSERT(unit, "null unit");

			m_bot.m_map.drawText(unit->pos, squad.getName(), sc2::Colors::Green);
		}
	}

	m_bot.m_map.drawTextScreen(sc2::Point2D(0.5f, 0.2f), ss.str(), sc2::Colors::Red);
}
