...
CombatCommander::CombatCommander(YoAgent & bot)
	: m_bot(bot)
	, m_squadData(bot)
	, m_initialized(false)
...
void CombatCommander::onStart()
{
	...
	SquadOrder idleOrder(SquadOrderTypes::Idle, m_bot.Observation()->GetStartLocation(), 5, "Chill Out");
	m_squadData.addSquad("Idle", Squad("Idle", idleOrder, IdlePriority, m_bot));

	// the main attack squad that will pressure the enemy's closest base location
	SquadOrder mainAttackOrder(SquadOrderTypes::Attack, getMainAttackLocation(), 25, "Attack Enemy Base"); //sc2::Point2D(0.0f, 0.0f), 25, "Attack Enemy Base");
	m_squadData.addSquad("MainAttack", Squad("MainAttack", mainAttackOrder, AttackPriority, m_bot));

	// the scout defense squad will handle chasing the enemy worker scout
	SquadOrder enemyScoutDefense(SquadOrderTypes::Defend, m_bot.Observation()->GetStartLocation(), 25, "Get the scout");
...
void CombatCommander::onFrame(const std::vector<const sc2::Unit *> & combatUnits)
{
	...
	{
		updateIdleSquad();
		//updateScoutDefenseSquad(); //transfer scout defense here, or add mineral-speedup take/release code
		updateDefenseSquads();
		updateAttackSquads();
	}
...
bool CombatCommander::shouldWeStartAttacking()
{
	...
	return m_combatUnits.size() >= 8; //m_bot.Config().CombatUnitsForAttack;
}
...
void CombatCommander::updateScoutDefenseSquad()
{
	...
	const BaseLocation * myBaseLocation = m_bot.m_baseLocationManager.getPlayerStartingBaseLocation(Players::Self);
	...
	if (scoutDefenseSquad.isEmpty() && assignScoutDefender)
	{
		...
		if (enemyWorkerUnit && workerDefender)
		{
			...
			{
				m_bot.m_workerManager.setCombatWorker(workerDefender);
	...
	else if (!scoutDefenseSquad.isEmpty() && !assignScoutDefender)
	{
		for (auto unit : scoutDefenseSquad.getUnits())
		{
			...
			if (Util::IsWorker(unit))
			{
				m_bot.m_workerManager.finishedWithWorker(unit);
			}
		}
...
void CombatCommander::updateDefenseSquads()
{
	...
	const BaseLocation * enemyBaseLocation = m_bot.m_baseLocationManager.getPlayerStartingBaseLocation(Players::Enemy);
	for (const BaseLocation * myBaseLocation : m_bot.m_baseLocationManager.getOccupiedBaseLocations(Players::Self))
	{
	...
	for (const auto & kv : m_squadData.getSquads())
	{
		...
		for (auto & unit : m_bot.Observation()->GetUnits(sc2::Unit::Alliance::Enemy))
		{
...
const sc2::Unit * CombatCommander::findClosestDefender(const Squad & defenseSquad, const sc2::Point2D & pos)
{
	...
	for (auto & unit : m_combatUnits)
	{
		...
		if (dist < minDistance) //first dist() will always be less than float.max , don't need "!closestDefender ||"
		{
...
sc2::Point2D CombatCommander::getMainAttackLocation()
{
	...
	if (enemyBaseLocation)
	{
		...
		if (!m_bot.m_map.isExplored(enemyBasePosition))
		{
		...
		else
		{
			...
			for (auto & enemyUnit : m_bot.m_unitInfoManager.getUnits(Players::Enemy))
			{
			...
			return enemyBasePosition; //go to enemyBase anyway
		}
	}

	// Second choice: Attack known enemy buildings
	for (const auto & kv : m_bot.m_unitInfoManager.getUnitInfoMap(Players::Enemy))
	{
	...
	for (auto & enemyUnit : m_bot.m_unitInfoManager.getUnits(Players::Enemy))
	{
		if (enemyUnit->unit_type != sc2::UNIT_TYPEID::ZERG_OVERLORD && !Util::IsWorkerType(enemyUnit->unit_type))
		{
	...
	return m_bot.m_map.getLeastRecentlySeenPosition();
}
...
const sc2::Unit * CombatCommander::findClosestWorkerTo(std::vector<const sc2::Unit *> & unitsToAssign, const sc2::Point2D & target)
{
	...
	for (auto & unit : unitsToAssign)
	{
		...
		if ( m_bot.m_workerManager.isFree(unit))
		{
