...
void DistanceMap::computeDistanceMap(YoAgent & m_bot, const sc2::Point2D & startTile)
{
	m_startTile = startTile;
	m_width = m_bot.m_map.width();
	m_height = m_bot.m_map.height();
	...
	for (size_t fringeIndex = 0; fringeIndex<fringe.size(); ++fringeIndex)
	{
		...
		for (size_t a = 0; a<LegalActions; ++a)
		{
			...
			if (m_bot.m_map.isWalkable(nextTile) && getDistance(nextTile) == -1)
			{
...
void DistanceMap::draw(YoAgent & bot) const
{
	...
	for (size_t i(0); i < tilesToDraw; ++i)
	{
		...
		bot.m_map.drawText(textPos, ss.str());
	}
}
