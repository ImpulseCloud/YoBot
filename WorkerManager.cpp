...
WorkerManager::WorkerManager(YoAgent & bot)
	: m_bot(bot)
	, m_workerData(bot)
{
...
void WorkerManager::onFrame()
{
	...
	//handleGasWorkers(); //TODO: transfer mineral-speedup into here
	//handleIdleWorkers(); //TODO: transfer mineral-speedup into here
...
{
	//if (!m_bot.Config().DrawResourceInfo) return;
	...
	for (auto & worker : m_workerData.getWorkers())
	{
		...
		m_bot.m_map.drawText(worker->pos, m_workerData.getJobCode(worker));

		auto depot = m_workerData.getWorkerDepot(worker);
		if (depot)
		{
			m_bot.m_map.drawLine(worker->pos, depot->pos);
		}
	}
}
...
{
	//if (!m_bot.Config().DrawWorkerInfo) return;
	return;
	...
	m_bot.m_map.drawTextScreen(sc2::Point2D(0.75f, 0.2f), ss.str());
}
