...
BaseLocationManager::BaseLocationManager(YoAgent & bot)
	: m_bot(bot)
{
...
{
	m_tileBaseLocations = std::vector<std::vector<BaseLocation *>>(m_bot.m_map.width(), std::vector<BaseLocation *>(m_bot.m_map.height(), nullptr));
	...
	for (auto mineral : m_bot.Observation()->GetUnits(sc2::Unit::Alliance::Neutral))
	{
		...
		for (auto & cluster : resourceClusters)
		{
			...
			if (dist < clusterDistance)
			{
				...
				float groundDist = dist; //m_bot.m_map.getGroundDistance(mineral.pos, Util::CalcCenter(cluster));
	...
	for (auto & geyser : m_bot.Observation()->GetUnits(sc2::Unit::Alliance::Neutral))
	{
		...
		for (auto & cluster : resourceClusters)
		{
			//int groundDist = m_bot.m_map.getGroundDistance(geyser.pos, Util::CalcCenter(cluster));
	...
	for (float x = 0; x < m_bot.m_map.width(); ++x)
	{
		for (int y = 0; y < m_bot.m_map.height(); ++y)
		{
			for (auto & baseLocation : m_baseLocationData)
			{
...
void BaseLocationManager::onFrame()
{
	...
	for (auto & unit : m_bot.Observation()->GetUnits(sc2::Unit::Alliance::Ally))
	{
		...
		if (!Util::IsBuilding(unit->unit_type) || unit->is_flying) continue;
	...
	for (const auto & kv : m_bot.m_unitInfoManager.getUnitInfoMap(Players::Enemy))
	{
		...
		if (!Util::IsBuilding(ui.type)) continue;
...
{
	if (!m_bot.m_map.isValid(pos)) { return nullptr; }
...
void BaseLocationManager::drawBaseLocations()
{
	...
	for (auto & baseLocation : m_baseLocationData)
	{
		...
		for (auto & baseLocation2 : m_baseLocationData)
		{
			if (baseLocation2.getGroundDistance(baseLocation.getDepotPosition()) < 70) {
				m_bot.m_map.drawLine(baseLocation.getDepotPosition(), baseLocation2.getDepotPosition());
			}
	...
	m_bot.m_map.drawSphere(nextExpansionPosition, 1, sc2::Colors::Purple);
	m_bot.m_map.drawText(nextExpansionPosition, "Next Expansion Location", sc2::Colors::Purple);
}
