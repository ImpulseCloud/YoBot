...
#include "Util.h"
//#include "CCBot.h" //replace with YoBot or YoAgent ??
#include "YoBot.h" //must include for m_bot.m_techtree reference, only
#include <iostream>
...
sc2::UnitTypeID Util::GetUnitTypeIDFromName(const std::string & name, sc2::Agent & bot)
{
...
sc2::UpgradeID Util::GetUpgradeIDFromName(const std::string & name, sc2::Agent & bot)
{
...
sc2::BuffID Util::GetBuffIDFromName(const std::string & name, sc2::Agent & bot)
{
...
sc2::AbilityID Util::GetAbilityIDFromName(const std::string & name, sc2::Agent & bot)
{
...
UnitTag GetClosestEnemyUnitTo(const sc2::Unit * ourUnit, const sc2::ObservationInterface * obs)
{
	...
	double closestDist = FLT_MAX; //std::numeric_limits<double>::max(); //template throwing compiler (C4003) errors for some reason
...
bool Util::UnitCanBuildTypeNow(const sc2::Unit * unit, const sc2::UnitTypeID & type, sc2::Agent & m_bot)
{
	//BOT_ASSERT(unit, "Unit pointer was null");
	...
	{
		//TODO: ignoring for now, fix later
		// check to see if one of the unit's available abilities matches the build ability type
		/*sc2::AbilityID buildTypeAbility = m_bot.m_techTree.getData(type).buildAbility;
